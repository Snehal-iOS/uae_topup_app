...
void main() {
  ...
  group('CheckTopupEligibilityUseCase', () {
    test('should return true when balance and limits allow top-up', () async {
      ...
      final result = await useCase(user: tUser, beneficiary: tBeneficiary, amount: 100.0);
    ...
    test('should return false when amount is zero', () async {
      ...
      final result = await useCase(user: tUser, beneficiary: tBeneficiary, amount: 0.0);
    ...
    test('should return false when amount is negative', () async {
      ...
      final result = await useCase(user: tUser, beneficiary: tBeneficiary, amount: -10.0);
    ...
    test('should return false when balance is insufficient', () async {
      ...
      final result = await useCase(user: userWithLowBalance, beneficiary: tBeneficiary, amount: 100.0);

      expect(result, isFalse);
    });

    test('should return false when beneficiary monthly limit would be exceeded', () async {
      final beneficiaryNearLimit = tBeneficiary.copyWith(monthlyTopupAmount: 900.0);
      when(mockUserRepository.getUser()).thenAnswer((_) async => tUser);
      when(mockBeneficiaryRepository.getBeneficiaries()).thenAnswer((_) async => [beneficiaryNearLimit]);

      final result = await useCase(user: tUser, beneficiary: beneficiaryNearLimit, amount: 200.0);

      expect(result, isFalse);
    });

    test('should return false when user total monthly limit would be exceeded', () async {
      final userNearTotalLimit = tUser.copyWith(monthlyTopupTotal: 2950.0);
      when(mockUserRepository.getUser()).thenAnswer((_) async => userNearTotalLimit);
      when(mockBeneficiaryRepository.getBeneficiaries()).thenAnswer((_) async => [tBeneficiary]);

      final result = await useCase(user: userNearTotalLimit, beneficiary: tBeneficiary, amount: 100.0);
    ...
    test('should use latest user and beneficiary from repositories', () async {
      ...
      final result = await useCase(user: tUser.copyWith(balance: 0), beneficiary: tBeneficiary, amount: 100.0);
