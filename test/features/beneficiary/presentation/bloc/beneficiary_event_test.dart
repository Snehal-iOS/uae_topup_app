  ...
  group('BeneficiaryEvent', () {
    ...
    group('AddBeneficiary', () {
      test('props should contain phoneNumber and nickname', () {
        const event = AddBeneficiary(phoneNumber: '+971501234567', nickname: 'Test');
        expect(event.props, equals(['+971501234567', 'Test']));
      });

      test('two instances with same values should be equal', () {
        const event1 = AddBeneficiary(phoneNumber: '+971501234567', nickname: 'Test');
        const event2 = AddBeneficiary(phoneNumber: '+971501234567', nickname: 'Test');
        expect(event1, equals(event2));
      });

      test('two instances with different values should not be equal', () {
        const event1 = AddBeneficiary(phoneNumber: '+971501234567', nickname: 'Test1');
        const event2 = AddBeneficiary(phoneNumber: '+971501234567', nickname: 'Test2');
    ...
    group('ToggleBeneficiaryStatus', () {
      test('props should contain beneficiaryId and activate', () {
        const event = ToggleBeneficiaryStatus(beneficiaryId: '123', activate: true);
        expect(event.props, equals(['123', true]));
      });

      test('two instances with same values should be equal', () {
        const event1 = ToggleBeneficiaryStatus(beneficiaryId: '123', activate: true);
        const event2 = ToggleBeneficiaryStatus(beneficiaryId: '123', activate: true);
        expect(event1, equals(event2));
      });

      test('two instances with different values should not be equal', () {
        const event1 = ToggleBeneficiaryStatus(beneficiaryId: '123', activate: true);
        const event2 = ToggleBeneficiaryStatus(beneficiaryId: '123', activate: false);
        expect(event1, isNot(equals(event2)));
      });
    });
    ...
    group('UpdateBeneficiaryMonthlyAmount', () {
      test('props should contain beneficiaryId and amount', () {
        const event = UpdateBeneficiaryMonthlyAmount(beneficiaryId: '123', amount: 100.0);
        expect(event.props, equals(['123', 100.0]));
      });

      test('two instances with same values should be equal', () {
        const event1 = UpdateBeneficiaryMonthlyAmount(beneficiaryId: '123', amount: 100.0);
        const event2 = UpdateBeneficiaryMonthlyAmount(beneficiaryId: '123', amount: 100.0);
        expect(event1, equals(event2));
      });

      test('two instances with different values should not be equal', () {
        const event1 = UpdateBeneficiaryMonthlyAmount(beneficiaryId: '123', amount: 100.0);
        const event2 = UpdateBeneficiaryMonthlyAmount(beneficiaryId: '123', amount: 200.0);
        expect(event1, isNot(equals(event2)));
      });
    });
